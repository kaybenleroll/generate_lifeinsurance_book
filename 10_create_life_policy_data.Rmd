...
prem_expense_mean <- -1.0
prem_expense_sd   <-  0.2  

premium_expense_ratio <- rlnorm(N, prem_expense_mean, prem_expense_sd)
...
load(paste0(input_dir, "/lifemorb_tables.rda"))
...
dt_smallarea <- fread(paste0(input_dir, "/smallarea_gisdb.csv"))[, sa_id := geogid]

# Load up the clustering info for the small areas
dt_clustersmallarea <- melt(fread(paste0(input_dir, "/aai_censusclusterids.csv")), id = 'sa_id')[
...
dt_clusterpolicyprop <- fread(paste0(input_dir, "/cluster_policy_proportions.csv"))

# Load the cluster propensities for the four product types
dt_clusterproductmapping <- fread(paste0(input_dir, "/cluster_product_mapping.csv"))

# Load product data
dt_productdata    <- fread(paste0(input_dir, "/product_input_data.csv"))
dt_productdurprop <- fread(paste0(input_dir, "/product_duration_data.csv"))
```
...
write_csv    (dt_output, path = paste0(output_dir, '/', output_csv))
write_feather(dt_output, path = paste0(output_dir, '/', output_feather))

# Convert the data to JSON and output it
write(jsonlite::toJSON(dt_output), file = paste0(output_dir, '/',output_json))

saveRDS(dt_output, file = paste0(output_dir, '/',output_rds),              compress = 'xz')
saveRDS(dt_output, file = paste0(output_dir, '/',output_intermediate_rds), compress = 'xz')
```
