...
```{r}
dt_policy[prod_type == 'protection'
                    ,lapse_month := calculate_lapse_month(.SD), by = policy_id]
```
...
```{r}
t(head(dt_policy[is.na(policy_status)]))
```

```{r}
dt_policy[, .N, by = lapsed]
```

```{r}
dt_policy[, .N, by = .(prod_type, lapsed)][order(prod_type, lapsed)]
```

```{r}
dim(dt_policy[prod_type == 'protection'][lapse_month == 0])
```

```{r}
t(dt_policy[is.na(lapsed)])
```
...
```{r}
dt_output[policy_statuschangedate != policy_startdate &
          policy_statuschangedate != policy_enddate][, .N, by = lapsed]
```

```{r}
t(head(dt_output[policy_statuschangedate > as.Date('2015-12-31')],3))
```
...
```{r}
write.csv(dt_output
          ,file = 'output/dt_policy_inoutforce.csv'
          ,row.names = FALSE
          ,fileEncoding = 'UTF-8');
```
...
```{r}
write(toJSON(dt_output), file = 'output/lifeins_policybook_inoutforce.json')
```
